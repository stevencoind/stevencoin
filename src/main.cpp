...
#if defined(NDEBUG)
# error "Stevencoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xf16551470d9108c4f94954d12b3c834d5f63f15befe3f4bd93ad9ed6f4896e12");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Stevencoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Stevencoin Signed Message:\n";
...
{
    // Stevencoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Stevencoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Stevencoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Stevencoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Stevencoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Stevencoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xad14439089130635a6f7358ea8fe0f9fb7973987bc196c785b7f4b0ba1ff5026");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Jessheim Times 08/Oct/2018 Steven make steven coin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04b59dca893c139e86de0cfc008c68fecd90b8395618ec963f386a7216510d4e2cd88dc387e5a81c8dbf637503494c51bb36d4a8bd5cbb5fb0776284981f8bce10") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1539027770;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084691505;

        if (fTestNet)
        {
            block.nTime    = 1539027697;
            block.nNonce   = 385915195;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x6410b0302e88eab79f23226a6eed5fd7921ff58d1d032c67e5a6e914b299b207"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Stevencoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("StevencoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("StevencoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static StevencoinMiner(CWallet *pwallet)
{
    printf("StevencoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("stevencoin-miner");
    ...
    try { loop {
        ...
        printf("Running StevencoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("StevencoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&StevencoinMiner, pwallet));
}
